...
| PV-14 | **DEFERRED — CP10 PRODUCTION HARDENING** | Server-side verify rate limiting / attempt caps. **Required before production release; not implemented yet.** **Not part of the CP1–CP9 acceptance gate** (Tasks 1–8 auth-boundary scope). | When implemented: repeat verify rapidly → `429` / rate-limited state; server caps attempts per IP/order | Network 429; server logs | Not implemented (deferred to CP10 production hardening) |
...
> **Note:** Server-side verify rate limiting (PV-14) is **required before production release** but is intentionally **not implemented yet** — deferred to **CP10 production hardening**. It is **not** part of the CP1–CP9 acceptance gate (Tasks 1–8 auth-boundary scope), so it is deliberately excluded from the P0 must-run shortlist above (20 scenarios).
...
> CP10 production hardening). It is a **pre-production hardening requirement**, not a CP1–CP9
...
10. **Server-side rate limiting / attempt caps** (PV-14) — **required before production release**; **intentionally not implemented yet** (deferred to **CP10 production hardening**). This is **not** a CP1–CP9 acceptance gate item (and not merely "unprovable by mocks").
